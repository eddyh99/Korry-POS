	...
	var table;
		...
		"footerCallback": function ( row, data, start, end, display ) {
			...
			var numVal = function (val) {
				return typeof val === 'string' ? parseFloat(val.replace(/[^0-9.-]/g, '')) || 0
					: (typeof val === 'number' ? val : 0);
			};

			// ambil hanya baris yang ter-filter (search / filter applied)
			var rowsFiltered = api.rows({ search: 'applied' }).data();

			// stok_awal sekarang di kolom 2
			var total_awal = api.column(2, { search: 'applied' }).data()
				.reduce(function(a, b){ return numVal(a) + numVal(b); }, 0);

			// kolom yang sebelumnya pakai render: hitung dari rowsFiltered
			var total_masuk = rowsFiltered.reduce(function(a, row){
				return a + (Number(row.produksi_in) + Number(row.pindah_in));
			}, 0);

			var total_keluar = rowsFiltered.reduce(function(a, row){
				return a + (Number(row.pindah_out) + Number(row.pinjam_out));
			}, 0);

			var total_jual = rowsFiltered.reduce(function(a, row){
				return a + (
			...
			var total_retur = rowsFiltered.reduce(function(a, row){
				return a + (Number(row.retur) + Number(row.retur_konsinyasi_in));
			}, 0);

			var total_konsinyasi = rowsFiltered.reduce(function(a, row){
				return a + (
			...
			// penyesuaian sekarang di kolom 8, sisa di kolom 9
			var total_sesuai = api.column(8, { search: 'applied' }).data()
				.reduce(function(a, b){ return numVal(a) + numVal(b); }, 0);

			var total_sisa = api.column(9, { search: 'applied' }).data()
				.reduce(function(a, b){ return numVal(a) + numVal(b); }, 0);

			// tulis ke footer (kolom 2..9)
